...
#replied with error of unused local variable.
...
#stating that it’s undefined. However, when you initialize a local variable within an if block, even 
#if that if block doesn’t get executed, the local variable is initialized to nil.
...
#in greetings. If instead of modifying the original object, we wanted to only modify 
# but not greetings, there are a couple of options:

#we could initialize informal_greeting with a reference to a new object containing the 
...
#condition, and you're not handling the case that there are more or 
