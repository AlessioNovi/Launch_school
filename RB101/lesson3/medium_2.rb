...
#by a particular named-variable can change depending on what is done to that named-variable.
...
#guessed correctly as per solution
...
#We simply took the contents of the block from the
#previous practice problem and moved it to a method, to which we are passing all of our outer variables.
#Predict how the values and object ids will change throughout the flow of the code below:
...
#the method returns a mew string withe += operator which doesn't modify 
#the caller while the << operator used on the array parameter inside the method
#actually modifies the caller 

#solution explanation

#Why? It seems clear from the above that Ruby 
#treats string and array arguments differently...
#but not so fast. Actually the arguments are treated in exactly the same way...but the results are different.

#In both cases, Ruby passes the arguments "by value", 
#but unlike some other languages, the value that gets 
#passed is a reference to some object. The string argument 
#is passed to the method as a reference to an object of type String. 
...
#When we attach an additional element to an array using the << operator, Ruby simply keeps using the same 
# that was passed in, and appends the new element to it.
...
#Here we have the same result inside method definiton but the operators have swapped operators (thus the effect outside the method are the opposite)
#in the case of our string the << operator will modify the outer string as well as our local 
#method variable will still point to the same object while for array the reassignment
#will create a new array with a different object that is connected only with the local 
#variable inside the method thus not altering the original argument.
...
#but the results are the opposite, because our method does the "same thing" to each, but in a different way than before. That is, in both cases, the method attempts to change the string from "pumpkins" 
#to "pumpkinsrutabaga" and add "rutabaga" to the array.
...
#when we change the object it points to. Does it create a new local 
#variable? Or is it able to just modify the existing object?
...
#looked at the solution this time. my idea was to create two methods
#one destructive (with a ! in order to warn of the destructive nature of the method)
#and one without. in the solution we're basically reasiggning our outer variables
#by calling on them our method which in turns modify our original variables as per 
#method definition.
